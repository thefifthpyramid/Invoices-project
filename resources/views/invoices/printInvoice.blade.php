                        ...
                        <div class="row mg-t-20">
                            ...
                            <div class="col-md">
                                ...
                                <p class="invoice-info-row"><span>تاريخ الاصدار</span>
                                    <span>{{ $invoice_data->invoice_Date }}</span>
                                </p>
                                <p class="invoice-info-row"><span>تاريخ الاستحقاق</span>
                                    <span>{{ $invoice_data->Due_date }}</span>
                                </p>
                                <p class="invoice-info-row"><span>القسم</span>
                                    <span>{{ $invoice_data->section->section_name }}</span>
                                </p>
                            </div>
                        </div>
                        <div class="table-responsive mg-t-40">
                            <table class="table table-invoice border text-md-nowrap mb-0">
                                ...
                                <tbody>
                                    <tr>
                                        <td>1</td>
                                        <td class="tx-12">{{ $invoice_data->product }}</td>
                                        <td class="tx-center">{{ number_format($invoice_data->Amount_collection, 2) }}</td>
                                        <td class="tx-right">{{ number_format($invoice_data->Amount_Commission, 2) }}</td>
                                        @php
                                            $total = $invoice_data->Amount_collection + $invoice_data->Amount_Commission;
                                        @endphp
                                        <td class="tx-right">
                                    ...
                                    <tr>
                                        <td class="tx-right">نسبة الضريبة ({{ $invoice_data->Rate_VAT }})</td>
                                        <td class="tx-right" colspan="2">287.50</td>
                                    </tr>
                                    <tr>
                                        <td class="tx-right">قيمة الخصم</td>
                                        <td class="tx-right" colspan="2">
                                            {{ number_format($invoice_data->Discount, 2) }}</td>

                                    </tr>
                                    <tr>
                                        <td class="tx-right tx-uppercase tx-bold tx-inverse">الاجمالي شامل الضريبة</td>
                                        <td class="tx-right" colspan="2">
                                            <h4 class="tx-primary tx-bold">{{ number_format($invoice_data->Total, 2) }}
                                            </h4>
                                        </td>
                                    </tr>
                                </tbody>
